...
void setup() {
  Serial.begin(115200);
  Logg.setLoggLevel(ERROR | CONNECTION | DEBUG);
  ...
  eventSTAGotIPHandler = WiFi.onEvent(
      [&](WiFiEvent_t event, WiFiEventInfo_t info) {
        ...
        Logg(CONNECTION, "eventSTAGotIPHandler: SYSTEM_EVENT_STA_GOT_IP\n");
  ...
  Logg(CONNECTION, "Beginning\n");
  mesh.addTask(TASK_SECOND, TASK_FOREVER, []() {
      Logg(CONNECTION, "Connected? %d\n", WiFi.status() == WL_CONNECTED);
      });
}
