...
extern painlessmesh::logger::LoggClass Logg;
...
class Mesh : public painlessmesh::Mesh<Connection> {
 public:
   ...
   * - Loggs on to the best mesh network node it finds… if it doesn’t find
  ...
  void init(TSTRING ssid, TSTRING password, uint16_t port = 5555,
    ...
    if (WiFi.softAPmacAddress(MAC) == 0) {
      Logg(ERROR, "init(): WiFi.softAPmacAddress(MAC) failed.\n");
    }
    uint32_t nodeId = tcp::encodeNodeId(MAC);
    if (nodeId == 0) Logg(ERROR, "NodeId set to 0\n");
  ...
  void tcpConnect() {
    ...
    if (WiFi.status() == WL_CONNECTED && WiFi.localIP()) {
      ...
      Logg(ERROR, "tcpConnect(): err Something un expected in tcpConnect()\n");
    }
  }
  ...
  void eventHandleInit() {
    ...
    eventSTAConnectedHandler = WiFi.onStationModeConnected(
        [&](const WiFiEventStationModeConnected &event) {
          // Logg(CONNECTION, "Event: Station Mode Connected to \"%s\"\n",
          // event.ssid.c_str());
          Logg(CONNECTION, "Event: Station Mode Connected\n");
        });

    eventSTADisconnectedHandler = WiFi.onStationModeDisconnected(
        [&](const WiFiEventStationModeDisconnected &event) {
          Logg(CONNECTION, "Event: Station Mode Disconnected\n");
          this->droppedConnectionCallbacks.execute(0, true);
        });
