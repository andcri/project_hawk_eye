...
void ICACHE_FLASH_ATTR StationScan::scanComplete() {
  using namespace painlessmesh::logger;
  Logg(CONNECTION, "scanComplete(): Scan finished\n");

  aps.clear();
  Logg(CONNECTION, "scanComplete():-- > Cleared old APs.\n");

  auto num = WiFi.scanComplete();
  if (num == WIFI_SCAN_FAILED) {
    Logg(ERROR, "wifi scan failed. Retrying....\n");
  ...
  Logg(CONNECTION, "scanComplete(): num = %d\n", num);
  ...
  for (auto i = 0; i < num; ++i) {
    ...
    Logg(CONNECTION, "\tfound : %s, %ddBm\n", record.ssid.c_str(),
        (int16_t)record.rssi);
  }

  Logg(CONNECTION, "\tFound %d nodes\n", aps.size());
...
void ICACHE_FLASH_ATTR StationScan::requestIP(WiFi_AP_Record_t &ap) {
  using namespace painlessmesh::logger;
  Logg(CONNECTION, "connectToAP(): Best AP is %u<---\n",
  ...
  if (aps.empty()) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (!layout::isRooted(mesh->asNodeTree()) && random(0, 1000) < prob) {
        Logg(CONNECTION, "connectToAP(): Reconfigure network: %s\n",
            String(prob).c_str());
