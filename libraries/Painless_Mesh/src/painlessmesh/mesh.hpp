...
class Mesh : public ntp::MeshTime, public plugin::PackageHandler<T> {
 public:
  ...
  void setDebugMsgTypes(uint16_t types) { Logg.setLoggLevel(types); }
  ...
  bool sendSingle(uint32_t destId, TSTRING msg) {
    Logg(logger::COMMUNICATION, "sendSingle(): dest=%u msg=%s\n", destId,
        msg.c_str());
  ...
  bool sendBroadcast(TSTRING msg, bool includeSelf = false) {
    using namespace logger;
    Logg(COMMUNICATION, "sendBroadcast(): msg=%s\n", msg.c_str());
  ...
  bool startDelayMeas(uint32_t id) {
    using namespace logger;
    Logg(S_TIME, "startDelayMeas(): NodeId %u\n", id);
  ...
  void onNewConnection(newConnectionCallback_t onNewConnection) {
    Logg(logger::GENERAL, "onNewConnection():\n");
  ...
  void onChangedConnections(changedConnectionsCallback_t onChangedConnections) {
    Logg(logger::GENERAL, "onChangedConnections():\n");
  ...
  void onNodeTimeAdjusted(nodeTimeAdjustedCallback_t onTimeAdjusted) {
    //Logg(logger::GENERAL, "onNodeTimeAdjusted():\n");
  ...
  void onNodeDelayReceived(nodeDelayCallback_t onDelayReceived) {
    //Logg(logger::GENERAL, "onNodeDelayReceived():\n");
 ...
 protected:
  ...
  void startTimeSync(std::shared_ptr<T> conn) {
    using namespace logger;
    Logg(S_TIME, "startTimeSync(): from %u with %u\n", this->nodeId,
        conn->nodeId);
    ...
    if (ntp::adopt(this->asNodeTree(), (*conn))) {
      timeSync = painlessmesh::protocol::TimeSync(this->nodeId, conn->nodeId,
                                                  this->getNodeTime());
      Logg(S_TIME, "startTimeSync(): Requesting time from %u\n", conn->nodeId);
    } else {
      timeSync = painlessmesh::protocol::TimeSync(this->nodeId, conn->nodeId);
      Logg(S_TIME, "startTimeSync(): Requesting %u to adopt our time\n",
          conn->nodeId);
    }
  ...
  void eraseClosedConnections() {
    using namespace logger;
    Logg(CONNECTION, "eraseClosedConnections():\n");
    this->subs.remove_if(
...
class Connection : public painlessmesh::layout::Neighbour,
 ...
 public:
  ...
  void initTasks() {
    ...
    Logg(CONNECTION, "painlessmesh::Connection: New connection established.\n");
