...
void routePackage(layout::Layout<T> layout, std::shared_ptr<T> connection,
  ...
  Logg(COMMUNICATION, "routePackage(): Recvd from %u: %s\n", connection->nodeId,
      pkg.c_str());
  ...
  if (calls == 0)
    Logg(DEBUG, "routePackage(): No callbacks executed; %u, %s\n", variant->type(), pkg.c_str());
}

template <class T, class U>
void handleNodeSync(T& mesh, protocol::NodeTree newTree,
                    std::shared_ptr<U> conn) {
  Logg(logger::SYNC, "handleNodeSync(): with %u\n", conn->nodeId);

  if (!conn->validSubs(newTree)) {
    Logg(logger::SYNC, "handleNodeSync(): invalid new connection\n");
    conn->close();
    return;
  }
  ...
  if (conn->newConnection) {
    ...
    mesh.addTask([&mesh, remoteNodeId]() {
      Logg(logger::CONNECTION, "newConnectionTask():\n");
      Logg(logger::CONNECTION, "newConnectionTask(): adding %u now= %u\n",
    ...
    conn->timeSyncTask.set(10 * TASK_SECOND, TASK_FOREVER, [conn, &mesh]() {
      Logg(logger::S_TIME, "timeSyncTask(): %u\n", conn->nodeId);
