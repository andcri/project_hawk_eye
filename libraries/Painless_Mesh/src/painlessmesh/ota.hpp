...
void addReceivePackageCallback(Scheduler& scheduler, plugin::PackageHandler<T>& mesh,
    ...
    if (currentFW->role == pkg.role && currentFW->hardware == pkg.hardware) {
      if ((currentFW->md5 == pkg.md5 && !pkg.forced) ||
        ...
        updateFW->task->setOnDisable([updateFW]() {
          Logg(ERROR, "OTA: Did not receive the requested data.\n");
          updateFW->md5 = "";
        });
      }
    }
  ...
  //   Logg(ERROR, "Data request should not be send to this node\n");
  //   return false;
  // });
  ...
  mesh.onPackage(12, [currentFW, updateFW, &mesh,
    ...
    if (updateFW->partNo == pkg.partNo && updateFW->md5 == pkg.md5 &&
...
#endif
        Logg(DEBUG, "Sketch size %d\n", maxSketchSpace);
        if (Update.isRunning()) {
          Update.end(false);
        }
        if (!Update.begin(maxSketchSpace)) {  // start with max available size
          Logg(DEBUG, "handleOTA(): OTA start failed!");
      ...
      if (Update.write((uint8_t*)b64Data.c_str(), b64Data.length()) !=
          b64Data.length()) {
        Logg(ERROR, "handleOTA(): OTA write failed!");
        Update.printError(Serial);
        Update.end();
        updateFW->md5 = "";
        updateFW->partNo = 0;
      ...
      if (pkg.partNo == pkg.noPart - 1) {
        ...
        if (Update.end(true)) {  // true to set the size to the
          ...
          if (!file) {
            Logg(ERROR, "handleOTA(): Unable to write md5 of new update to the SPIFFS file. This will result in endless update loops for OTA\n");
          }
        #else
          auto file = LittleFS.open(updateFW->ota_fn, "w");
          if (!file) {
            Logg(ERROR, "handleOTA(): Unable to write md5 of new binary to the LittleFS file. This will result in endless update loops for OTA\n");
          }
        #endif
          ...
          Logg(DEBUG, "handleOTA(): OTA Success! %s, %s\n", msg.c_str(),
              updateFW->role.c_str());
          ESP.restart();
        } else {
          Logg(DEBUG, "handleOTA(): OTA failed!\n");
          Update.printError(Serial);
          updateFW->md5 = "";
          updateFW->partNo = 0;
        }
